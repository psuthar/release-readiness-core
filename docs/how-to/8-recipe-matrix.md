...
> **Combining stacks is hand-merge work.** `release-readiness-init --stack <X>` writes one stack's evidence-collection block. The sample app's workflow combines the **Go test** snippet above with the **Go cover + LCOV** snippet below — both contribute steps to the same `collect-evidence` job. There is no `--stack go+go-coverage` flag; concatenate the two snippets in your workflow.
