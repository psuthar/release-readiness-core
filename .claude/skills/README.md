...
| Skill | When to use |
|-------|-------------|
| **`epic-run/`** | User says **`run epic <EPIC-KEY>`**, **`continue epic <EPIC-KEY>`**, or **`continue epic run for <EPIC-KEY>`**. Executes all epic children with **FULL_AUTO** using PR Gate success + mergeability rules; **`continue`** must **drain all remaining Jira work**, not a single ticket. If you **squash-merge yourself** after a halt, **`continue epic`** reconciles **Jira Done**, **`main`**, and **branch cleanup**—see `epic-run/SKILL.md` (**User override**). |
...
**Epic vs `CLAUDE.md` §8:** Standalone **`implement <TICKET-KEY> FULL_AUTO`** and epic runs both require PR Gate success + `mergeable_state: clean`; epic adds sequencing/resume rules. See **`CLAUDE.md` §10** and **`epic-run/SKILL.md`**.
