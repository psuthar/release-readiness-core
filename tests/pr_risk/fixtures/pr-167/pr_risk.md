...
**Base ref:** `8d4d260e5af6e8c13e2f29f6dc0431d840425eed`  
...
- Confirm scope with author: PR text implies domains rag but diff may differ.
...
- **Mode:** `focused` — Most churn (~58%) sits under `eval/qa`.
- Top area: `eval/qa` (~58% of churn); **3** distinct path prefixes.

### Hotspots (recent git activity)

- **`eval/qa`** — 5 distinct commits (sampled) — Prefix touched in 5 of the last 50 sampled commits — sustained activity; extra regression care.

### PR intent vs diff

- **Intent strength:** `strong`
- **Subject line (source):** SCRUM-141: extend expected_scores coverage to FF-026
- **Keywords matched:** rag
- **Domains implied by text:** rag
...
**Score:** 10.0/100 (low) · base `8d4d260e5af6e8c13e2f29f6dc0431d840425eed`
