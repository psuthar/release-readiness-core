...
**Base ref:** `f65f8d88a5d5eac962958a8f0d020ae10dae62e5`  
...
| Files changed | 9 |
| LOC churn (add+del) | 214 |
| Test files in diff | 4 |
| Config-ish files (CI/deploy/mod) | 0 |
| Validation note | yes (Validation: web vitest suite (394 tests) PASS; prrisk score 6.0 (low).) |
...
| `context_hotspot_regression_focus` | ✅ pass | git_signals | Validation note present in commit: Validation: web vitest suite (394 tests) PASS; prrisk score 6.0 (low). |
...
- **Mode:** `focused` — Most churn (~99%) sits under `web/src`.
- Top area: `web/src` (~99% of churn); **2** distinct path prefixes.

### Hotspots (recent git activity)

- **`web/src`** — 37 distinct commits (sampled) — Prefix touched in 37 of the last 50 sampled commits — sustained activity; extra regression care.
- **`web/tests`** — 24 distinct commits (sampled) — Prefix touched in 24 of the last 50 sampled commits — sustained activity; extra regression care.

### PR intent vs diff

- **Intent strength:** `strong`
- **Subject line (source):** SCRUM-223: scope decision-maker readiness row and add DM chip in stance tooltips
- **Keywords matched:** ci
...
- **Points:** 4.0
- **Detail:** Prefix touched in 37 of the last 50 sampled commits — sustained activity; extra regression care.
...
- Evidence: 55% of LOC churn is in test files
...
**Score:** 6.0/100 (low) · base `f65f8d88a5d5eac962958a8f0d020ae10dae62e5`
...
**Top risk drivers:**
- Diff overlaps a path prefix touched in multiple recent commits (4 pts): Prefix touched in 37 of the last 50 sampled commits — sustained activity; extra regression care.
